...
SELECT country,FORMAT(SUM(daily_new_deaths),2) as Deaths FROM covid_deaths WHERE daily_new_deaths> 0 GROUP BY country ORDER BY SUM(daily_new_deaths) DESC LIMIT 10;

#Case Growth Over Time to observe how cumulative cases and deaths change yearly.
SELECT year(date) as Year,FORMAT(SUM(cumulative_total_cases),2) AS Cases,FORMAT(SUM(cumulative_total_deaths),2) AS Deaths from covid_deaths group by year(date) ORDER BY year;
